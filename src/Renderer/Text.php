 ...
 */
namespace Snono\ReportBuilder\Renderer;
...
class Text extends AbstractRenderer
{
    ...
    function render(Frame $frame)
    {
        ...
        if($this->_dompdf->getOptions()->getLanguage() == 'ar') {

            if (!class_exists('I18N_Arabic')) {
                require_once(DOMPDF_DIR . "/lib/I18N/Arabic/Glyphs.php");
                $Arabic = new I18N_Arabic_Glyphs('AutoSummarize');
                $text = $Arabic->utf8Glyphs($text, 150);

            } else {
                Log::info('lang: ' . $this->_dompdf->getOptions()->getLanguage());
